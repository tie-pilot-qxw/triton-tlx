...
int lookupNumThreadsPerWarp(Operation *op) {
  std::optional<int> threadsPerWarp = maybeLookupAttr(op, AttrNumThreadsPerWarp);
  if (!threadsPerWarp) {
    op->emitOpError(
        "is not contained within a context that specifies the number of threads per warp");
    llvm::report_fatal_error("failed to lookup the number of threads per warp, the "
...
class SparseBlockedToMMA : public RewritePattern {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // TODO(sparsity) look more closely at CTA Layout. It should match A's CTA Layout?
    meta_type =
        RankedTensorType::get(meta_type.getShape(), meta_type.getElementType(),
                              SparseDotMetaEncodingAttr::get(context, mma_enc, mma_enc.getCTALayout()));
    ...
    if (triton_mma_version == 2) {  // MMAV2
      ...
      newDot = rewriter.create<SparseDotOp>(dotOp.getLoc(), newRetType,
                                                a, b, new_acc, meta);
    } else {  // MMAV3
...
struct SparseRemoveLayoutConversionPass
  ...
  void runOnOperation() override {
    getOperation().walk([&](triton::gpu::ConvertLayoutOp op) {
      ...
      auto order = cast<DistributedEncodingTrait>(src_encoding).getDefaultOrder();

      auto shared_layout = builder.getAttr<triton::gpu::SwizzledSharedEncodingAttr>(
