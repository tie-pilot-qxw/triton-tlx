...
def _tlx_fwd_inner(
    ...
    # v2_tile = tlx.local_view(v2_tiles, buf_id)  # [BLOCK_SIZE_KV, HEAD_DIM]
...
def _triplet_tlx_fwd_kernel(
    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task("default"):
            ...
            k1o_offset = (
                offs_b * (seq_len * num_kv_heads) + kv1_idx_start * num_kv_heads
            )
            ...
            acc_cnt = 0
            kv2_offset_y_start = (
                offs_b * (seq_len * num_kv_heads) + kv2_start * num_kv_heads
            )
        ...
        with tlx.async_task(
            ...
            for kv1_idx in tl.range(0, num_of_kv1_trips):
                ...
                k1_tile_rmem = tlx.local_load(k1_tile)  # [1, HEAD_DIM]
                v1_tile_rmem = tlx.local_load(v1_tile).to(tl.float32)

                qk1_tile_rmem = q_tile_rmem_scaled * k1_tile_rmem
                k1_full = tlx.local_view(k1_fulls, 0)

                # if q_idx == 1 and offs_b == 0 and kv1_idx == 1:
                #     tl.device_print("qk1_tile [TLX]", qk1_tile_rmem)

                # loop over k, v and update accumulator
                kv2_trip_cnt = 0
                for kv2_idx in tl.range(kv2_start, kv2_end, BLOCK_SIZE_KV):
                    # HAS_QK_MASK = has_qk_mask and kv2_trip_cnt == num_kv2_trips - 1
                    k2_tile2 = tlx.local_view(k2_tiles, 0)
                    # k2_tile = tlx.local_trans(k2_tile)  # [HEAD_DIM, BLOCK_SIZE_KV]

                    acc, m_i, l_i = _tlx_fwd_inner(
                        # qk1_tile_rmem,
                        # v1_tile_rmem,
                        acc,
                        l_i,
            ...
            acc = acc / l_i[:, None]
            qo_offset_ysplit = (
                offs_b * (seq_len * num_heads) + q_idx * num_heads + cid * BLOCK_M_SPLIT
            )
...
# from triplet_fwd_eager import triplet_attn_fwd_ref
# from triplet_fwd_triton import (
#     block_local_triplet_attn_fwd_gqa_pack as triplet_fwd_triton_ref,
# )

# from utils import compute_sqnr
...
def _test():
    ...
    v2 = torch.randn_like(v1).normal_(mean=0.0, std=0.5)

    # out_eager_ref, _ = triplet_fwd_triton_ref(q, k1, k2, v1, v2, w1, w2)
    # out_triton_ref, _ = triplet_fwd_triton_ref(q, k1, k2, v1, v2, w1, w2)
    out, _ = block_local_triplet_attn_fwd_gqa_pack(q, k1, k2, v1, v2, w1, w2)

    # err_0 = compute_sqnr(out_eager_ref, out)
    # err_1 = compute_sqnr(out_eager_ref, out_triton_ref)

    # print(out_eager_ref[-1][2][-1])
    # # print(out_triton_ref[-1][-1][-1])
    # print(out[-1][2][-1])

    # print(k2[-1][0][-1][101])

    # for i in range(seq_len):
    #     err = compute_sqnr(out_eager_ref[0][i], out[0][i])
    #     print(f"Triplet Test: {err=} {i=}")
    #     if err < 30:
    #         break

    # print(f"Numeric Test: {err_0=} {err_1=}")


def _bench():
    bs, seq_len, num_heads, head_dim = 4, 8192, 128, 128
    num_kv_heads = 1
    ...
    def _total_tflops():
        ...
        # Triplet: 2 multiplies + 1 add = 3
        QK_GEMM = bs * num_heads * seq_len * head_dim * w1 * w2 * 3
        PV_GEMM = QK_GEMM
        return (QK_GEMM + PV_GEMM) / 1e12
    ...
    fn = lambda: block_local_triplet_attn_fwd_gqa_pack(q, k1, k2, v1, v2, w1, w2)

    ms = triton.testing.do_bench_cudagraph(fn)

    secs = ms * 1e-3

    tflops = _total_tflops() / secs

    print(f"{bs=} {seq_len=} {num_heads=} {head_dim=} {w1=} {w2=}")
    print(f"Triplet TLX WS: {tflops} TFLOPS")
